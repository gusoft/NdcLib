...
export class NdcPackage {
    ...
    private testNDC(ndc: string): void {
        ...
        if (!this.ndcTest.test(ndc)) {
            throw new Error(`Invalid NDC. Format must be 4444-4444-22, 55555-333-22, 55555-4444-1.`);
        }
    }
    ...
    public product() {
        const productNdc = `${this.ndc[0]}-${this.ndc[1]}`;
        return new NdcProduct(productNdc);
    }
